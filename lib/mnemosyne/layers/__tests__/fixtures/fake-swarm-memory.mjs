...
// Real regression coverage for the 2026-09-08 "recall() never passed
// --scope" bug: when set, every invocation appends its own full argv
// (minus the node/script path) to this file, one JSON array per line --
// lets a test assert on the REAL args the adapter constructed, not just
// the output it parsed (which is how the original bug went unnoticed).
if (process.env.FAKE_SWARM_MEMORY_ARGV_LOG) {
  const fs = await import('node:fs');
  fs.appendFileSync(process.env.FAKE_SWARM_MEMORY_ARGV_LOG, JSON.stringify(process.argv.slice(2)) + '\n');
}
