...
log ""
log "Embedding Mnemosyne as a product's own memory agent, not a developer's own"
log "harness? Add --build (also opt-in, also printed-not-run by \`agent init\`"
log "itself) to also run a first-time index of your codebase (Layer 1 sync,"
log "persona seed, file/graph index) in the same step:"
log ""
log "  mnemosyne agent init --build"
log ""
log "See docs/embedded-layers.json for a recommended mnemosyne.layers.json for a"
log "bare embedded install with no swarm-memory credential configured."
log ""
log "Two explicit install paths (README.md's Quickstart names the same fork;"
log "design-discussion.md calls them Mode B / Mode A -- same concepts, not a"
log "third, unmapped term):"
log ""
log "  Sidecar / embedded (Mode B):  mnemosyne agent init --build [--storage-dir <dir>]"
log "  Full / system (Mode A):       mnemosyne agent init  (no --build, registers your own harness)"
log "                                 mnemosyne onboard <path> --collection <name> [--create]"
log ""
log "Run \`mnemosyne agent status\` to see which path (if either) has already run"
log "for this repo."
