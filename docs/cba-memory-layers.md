...
**Layer 3 (indexed docs): also Graphify — RESOLVED, confirmed, not just inferred (2026-08-14).** Real PoC: installed the actual PyPI package (`uv tool install graphifyy`, CLI `graphify` + an `graphify-mcp` MCP server binary — both real, both trivial to install), ran `graphify update .` against this repo twice independently (once by a fork, once by me directly) — same result both times: 1297 nodes / 2123 edges / 94 communities in ~3.9s wall-clock, **no LLM required** ("Re-extracting code files... no LLM needed" per its own output). Verified directly against the raw `graph.json`: 360 real markdown document nodes, each carrying a genuine `source_location` (e.g. `"L3"`, `"L17"`, `"L32"`, `"L64"`) via deterministic AST-level markdown parsing (`_origin: "ast"`) — **line-number addressability for docs does not require the LLM-assisted path at all**, contrary to this document's original assumption. The LLM path (Gemini/Claude/etc. via `GEMINI_API_KEY` or similar) is an optional enhancement for deeper cross-file semantic linking, not a requirement for the layer-3 use case. `graphify explain <node>` was tested live and returned real file:line-pointer results cross-checked against this session's own known-true facts — including correctly flagging the `MnemosyneClient` class-vs-interface ambiguity independently surfaced earlier this session.

**New finding, not in the original tool sweep: Graphify ships real cross-repo graph merging** (`graphify global add/remove/list`, `graphify merge-graphs`, writing to `~/.graphify/global-graph.json`) — directly relevant to the operator's three-tier orchestrator/company-director/area-architect hierarchy question (see the follow-on multi-agent research CBA for how this plugs in). Not yet evaluated in depth; flagged for the next pass.
...
1. ~~Verify Graphify's doc-indexing addressability directly~~ — **done, see above.**
2. **Evaluate Graphify's `--global`/`merge-graphs` cross-repo capability** against the three-tier orchestrator hierarchy (see the follow-on large-scale multi-agent research CBA, launched 2026-08-14, covering 50+ additional tools + this exact question).
3. **Decide whether to also fire off the async "fire off / poll / request more" pattern as a first-class Mnemosyne capability** — raised mid-session (2026-08-14): the operator flagged that the job-lifecycle shape used for the Gemini research call (submit, get a handle, poll status, request more) mirrors "how we deploy it, how we index, re-index, fire off those, inject, read, request" from their original vision. Not yet scoped as a story — worth a dedicated design pass, not bundled silently into the Graphify integration.
4. This document does not include an implementation plan/epic — that's the next step once the above items (and the larger follow-on research CBA) are resolved, not before.
