...
export class MetaLayer {
    constructor() {
        this.name = 'meta';
        this.scope = 'meta';
        this.directory = path.join(os.homedir(), '.claude', 'hive', 'vault');
    }
    ...
    async index() {
        ...
        try {
            ...
            console.error(`[mnemosyne] Failed to index meta layer:`, e);
            throw e;
        }
    }
    ...
    async recall(query, opts = {}) {
        if (!(await this.isAvailable())) {
            const err = new Error('Meta layer is unavailable');
