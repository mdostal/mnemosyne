...
## §8. Resolution (post real-mockup swarm review, supersedes §2.1's original hypothesis)

§2.1 above hypothesized grouped collapsible `<details>` clusters as the recommended IA direction, on paper, before any real mockup existed. A real multi-agent swarm review (`.pHive/design/mnemosyne-ui-redesign/{options,critiques,synthesized}/`, then a second adversarial re-judge round after the operator flagged that fixable implementation bugs were unfairly penalizing two of the three directions, `.pHive/design/mnemosyne-ui-redesign/round2/`) tested that hypothesis against real, rendered, interactive HTML against two genuine structural alternatives, and found it did **not** hold up: the collapsible-cluster direction bundled Liveliness/Settings/Operations behind one collapse toggle with (initially) zero status signal when collapsed, shipped fully open by default (so it doesn't even reduce length out of the box), and even after hardening (status dots, restored headings) it caps out at 3 bits of status resolution for 8 panels and introduced a new real accessibility regression (broken form-label associations) during the hardening pass itself.

**Actual confirmed direction, operator-confirmed 2026-08-18 ("cool, go for it, now hive plan and execute and nail it fully"):** a flat, always-visible document (structurally close to the app's own real current layout — no tabs, no collapsing of Liveliness/Settings/Operations/Lanes/Search/Graph/Memory Levels) with a sticky jump-chip nav bar carrying real per-panel status rings (sourced live from each panel's own status text, never fabricated), one narrowly-scoped `<details>` collapse limited to Personas' own write-tooling tail (draft form + Retrieval Layer Stack + Level 0 pointer — never the read-oriented identity table), the reconciled amber-gold accent (§2.2, unchanged), the real shipped favicon embedded inline in the header (not only as a browser tab icon), and the graph node's selected/focus states deliberately kept on the original `--fail`/`--pass` red/green values (an explicit operator preference, confirmed after both round-1 and round-2 review — the code must document this as a real choice, not silently invent "dedicated" tokens that alias back to the same values while claiming otherwise, which is a defect round 2 caught in the reference mockup).

Real source of truth for build-out: `.pHive/design/mnemosyne-ui-redesign/round2/candidates/hybrid-rings-original-graph-colors.html` — a mockup, not production code, but every structural/visual decision below derives from it plus round 2's own punch list of what it still gets wrong (see round2 critiques, especially `accessibility.md` and `visual-identity-cohesion.md`).
