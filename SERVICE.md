...
| `POST /conversation-memory/sources/scan` | —                                | cm-15: the Discovery & Pilot panel's "crawl" button — `execFile()`s `bin/mnemosyne-conversation-discover.mjs --json` (a `tsx`-launched wrapper around cm-02's `discoverSources({write:true})`), **fresh every call**, never a cached read. Returns the real manifest (`sessions[]`/`excluded[]`/`exports`) |
| `POST /conversation-memory/pilot/run` | `{sessionPaths: [...], exportKeys: [...]}` | cm-15: invokes cm-08's already-shipped `bin/mnemosyne-conversation-pilot.mjs` (`--sessions`/`--exports`/`--confirm`/`--json`) over **exactly** the given marked subset — `400` on an empty combined selection, before cm-08's orchestrator is ever invoked. `exportKeys` values are real ChatGPT conversation ids (cm-08's real `--exports` contract), not export-file-level keys. cm-08's own small-sample cap (max 5 per source) is never re-capped here — an oversized selection surfaces as cm-08's own real refusal, verbatim, in the response |
| `GET /conversation-memory/triage-queue` | — | cm-16: reads + classifies every line of the shared `~/.mnemosyne/conversation-triage-queue.jsonl` — quarantine hits (`quarantine_reason: 'secret_detected'`) and existing scope-route confirmations (`confirmation_reason: 'scope_route_confirmed'`), by their own discriminator field. A missing file returns `{quarantine: [], confirmations: []}` with `200`, never `404`/`500`. `cm-05`'s own keep/trash/uncertain triage verdicts (a different record kind in the same file) are deliberately not classified into either array — out of this route's scope |
| `GET /conversation-memory/intake-candidates` | — | cm-16: `execFile()`s a new read-only `mnemosyne/inventory/qdrant_inventory.py intake-candidates` CLI verb (`scroll_points('conversation_memory_intake')`), then reuses `distributeIntakeEntries.ts`'s own `partitionPoints()`/`readScopeRouteConfirmations()` (via the new, exported `computeIntakeCandidateStatuses()`) to tag every real candidate with exactly one status: `no_candidate` / `candidate_unconfirmed` / `candidate_confirmed_pending_distribution` / `distributed` |
| `POST /conversation-memory/scope-route/confirm` | `{cluster_id, scope_key}` | cm-16: the **ONE** real filesystem write in this story's entire route surface — one `fs.appendFileSync()` of a real `ScopeRouteConfirmationEntry` line to the shared triage-queue file, never a read-modify-rewrite. Refuses (`400`, zero writes) unless the named pair matches a currently-known `candidate_unconfirmed` row (re-reading the SAME real candidate statuses `GET .../intake-candidates` uses); a duplicate confirm of an already-confirmed pair **succeeds** (a harmless second line), never refused |
